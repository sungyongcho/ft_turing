...
let usage_msg =
  ...
  let () =
    ...
    try
      ...
      (* print_endline (Yojson.Safe.pretty_to_string json_data) *)
      let name = json_data |> Yojson.Safe.Util.member "name" |> Yojson.Safe.Util.to_string in
      print_endline ("Machine name: " ^ name);
    with
